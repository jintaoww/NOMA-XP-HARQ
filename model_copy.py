...
class ActorNet(nn.Module):
    def __init__(self, state_dim, a_dim, action_bound=1):
        ...
        self.fc1 = nn.Linear(np.prod(state_dim), 300)
        nn.init.normal_(self.fc1.weight, 0., 0.1)
        nn.init.constant_(self.fc1.bias, 0.1)
        ...
        nn.init.normal_(self.fc4.weight, 0., 0.1)
        nn.init.constant_(self.fc4.bias, 0.1)
        self.fc5 = nn.Linear(30, np.prod(a_dim))
        nn.init.normal_(self.fc5.weight, 0., 0.1)
        nn.init.constant_(self.fc5.bias, 0.1)
...
class CriticNet(nn.Module):
    def __init__(self, s_dim, a_dim, name):
        ...
        self.name = name # 可选的name属性
        self.fc1 = nn.Linear(np.prod(s_dim)+np.prod(a_dim), 300)
        nn.init.normal_(self.fc1.weight, 0., 0.1)
        nn.init.constant_(self.fc1.bias, 0.1)
        self.fc2 = nn.Linear(300, 200)
        nn.init.normal_(self.fc2.weight, 0., 0.1)
    ...
    def forward(self, s, a):
        ...
        #     file.write("\n")
        #     file.write(f"name is: {self.name}, x is: {x}")
        return x
