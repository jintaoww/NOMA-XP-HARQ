...
from test_noma_harq_env_oneagent import NOMA_Environ
...
parser.add_argument("--task", type=str, default="TD3_Power_outage_train")
...
parser.add_argument('--slot-per-test', type=int, default=10000)
...
if __name__ == '__main__':
    ...
    phi=10**-2
    power_db = 30
    env = NOMA_Environ(power_db, max_K, args.seed, phi=phi, beta=[18.7, 18.7], training=True)
    train_envs = NOMA_Environ(power_db, max_K, args.seed, training=True)
    test_envs = NOMA_Environ(power_db, max_K, args.seed, training=False)
    ...
    for e in range(args.epoch):
        ...
        while not d:
            ...
            a = agent(state).act[0]
            a = agent.exploration_noise(act=a.detach().numpy(), batch=state)
            s_, r, d, info = env.step(a)
            buffer.add(Batch(obs=s, act=a, rew=r, done=d, obs_next=s_, info={}))
            if (buffer.__len__() >= args.batch_size) and (j % 5 == 0):
                args.exploration_noise *= 0.9999
                agent.set_exp_noise(GaussianNoise(sigma=args.exploration_noise))
                agent.update(sample_size=args.batch_size, buffer=buffer)
            s = s_
            ep_reward += r
    ...
    # buffer = PrioritizedReplayBuffer(args.buffer_size, 0.3, 0.4)
