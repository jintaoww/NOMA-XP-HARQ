M = csvread('D:\CodeProject\PythonProject\NOMA_HARQ_DRL_FL\code\Twooutage_data.csv')
y1 = M(2, :)
y2 = M(3, :)
y3 = M(4, :)
y4 = M(5, :) 
...
y9 = M(7, :)
x = [10,15,20,25,30, 35];
semilogy(x, y1, 'b-.', x, y2, '->', x, y3, '-*', x, y4, '--s', x, y5, x, y9, 'k-h','linewidth', 2)
axis([10,35,2e-4,2e-1])
...
ylabel('Outage probability $~p^{out}_{2,K}~$','Interpreter','latex')
