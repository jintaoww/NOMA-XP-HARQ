...
class NOMA_Environ(gym.Env):
    ...
    # 初始化环境参数
    def __init__(self, max_K=3, seed=123, epsilon=10**-3, beta=[50, 50], training=False):
        np.random.seed(seed)
        self.se = seed
        self.time_step = 0.1  # 更新车辆位置信息的时间间隔
        self.vehicles = []  # 用于存储摆放在环境中的车辆对象（有Vehicle类生成）
        self.PO = 300.
        self.sig2_dB = -20  # 环境高斯噪声功率（dBm）
        ...
        self.epsilon = np.array([epsilon]*self.n_Veh)
        ...
        self.action_space = spaces.Box(low=-1, high=1, shape=(self.n_Veh * 1,), dtype=np.float64)
        # The observation will be the coordinate of the agent
        # this can be described both by Discrete and Box space
        self.observation_space = spaces.Box(low=-1000, high=1000, shape=(self.n_Veh * 3,), dtype=np.float64)
    ...
    def renew_channels_fast_fading(self):
        ...
        # print(f"self.V2I_channels_with_fast_fading is:{self.V2I_channels_with_fast_fading}")
    ...
    def step(self, action):
        ...
        action_temp = action_temp.reshape(self.n_Veh, -1)
        s = self.s_.copy()
        # rate = action_temp * [[2], [60]]
        rate = action_temp * 10
        # rate = action_temp[:,:1] * 50
        power = np.array([[1/3], [1/3]]) * self.PO
        ...
        if self.training:
            for i in range(self.n_Veh):
                if self.harq_time[i][0] >= 1:
                    reward_sums[i] = reward_sums[i] - self.lambd[i] * outage_time[i][0]
        
        if self.steps % 10 == 0:
            for i in range(self.n_Veh):
                if self.outage_num[i][0]/self.harq_time[i][0] - self.epsilon[i] > 0:
                    self.lambd[i] += self.beta[i] * np.log10(self.outage_num[i][0]/self.harq_time[i][0] / self.epsilon[i])
                else:
                    self.lambd[i] += 0

        if self.harq_time.all() >= 1:
            self.s_ = np.hstack((self.accumulation_rate, self.mutual_information, self.out_g))
        else:
            self.s_ = np.hstack((self.accumulation_rate, self.mutual_information, self.out_g))
        ...
        if self.steps >= 10000:
            print(f"outage_num is:{self.outage_num/self.harq_time}")
            done = True
        self.steps += 1
        # print(f"accumulation_power is:{self.accumulation_power}, cur_accumulation_power is:{cur_accumulation_power}, s is:{s}, a is:{action_temp}, r is:{reward_sums}, s_ is:{self.s_}, tras_k is:{tras_k}, dis is:{self.vehicles[0].distance, self.vehicles[1].distance}")
        return self.s_.reshape(-1), np.array([sum(reward_sums)]), done, info
    ...
    def reset(self):
        ...
        return np.hstack((self.accumulation_rate, self.mutual_information, self.V2I_channels_with_fast_fading[:, :1])).reshape(-1)