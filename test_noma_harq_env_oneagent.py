...
class NOMA_Environ(gym.Env):
    ...
    def __init__(self, power_db=20, max_K=3, seed=123, phi=10**-3, epsilon=0.2, beta=[50, 50], alpha=[1.5, 1.5], training=False):
        ...
        self.mutual_information = np.zeros((self.n_Veh, 1)) * 1e-10
        self.accumulation_rate = np.zeros((self.n_Veh, 1)) * 1e-10
        ...
        self.observation_space = spaces.Box(low=-1000, high=1000, shape=(6,), dtype=np.float64)
    ...
    def step(self, action):
        ...
        :return:
        """
        action_temp = action.copy().reshape(2, -1)
        ...
        for i in range(self.n_Veh):
            if self.current_transmission_time[i][0] < self.Maximum_number_of_transmission_K and reward_sums[i] <= 1e-10:
                ...
                self.mutual_information[i][0] = 1e-10
                self.accumulation_rate[i][0] = 1e-10
        ...
        return self.s_.reshape(-1), sum(reward_sums), done, info
    ...
    def reset(self):
        ...
        # self.add_new_vehicles_by_number(self.n_Veh, self.start_velocity)
        self.renew_channels_fast_fading()
        self.mutual_information = np.ones((self.n_Veh, 1)) * 1e-10
        self.accumulation_rate = np.ones((self.n_Veh, 1)) * 1e-10
        self.current_transmission_time = np.ones((self.n_Veh, 1))
        ...
        # out_gain = out_gain.reshape(-1)
        # out_gain = np.array([out_gain, out_gain])
        return np.hstack((self.accumulation_rate, self.mutual_information, out_gain)).reshape(-1)