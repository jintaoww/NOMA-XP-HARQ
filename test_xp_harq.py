...
class V2IChannels:
    ...
    def __init__(self, n_veh, n_rb):
        ...
        self.h = 1/np.sqrt(2) * (np.random.normal(size=(self.n_Veh, self.n_RB )) +
    ...
    def update_fast_fading(self):
        # self.h = 1/np.sqrt(2) * (np.random.normal(size=(self.n_Veh, self.n_RB )) +
        #                     1j * np.random.normal(size=(self.n_Veh, self.n_RB)))
        self.h = self.rho * self.h + np.sqrt(1 - np.power(self.rho, 2)) * np.random.normal(size=(self.n_Veh, self.n_RB))
        self.FastFading = np.abs(self.h)
        # self.FastFading = 20 * np.log10(np.abs(self.h))
...
class NOMA_Environ(gym.Env):
    ...
    # 初始化环境参数
    def __init__(self, max_K=3, seed=123):
        # np.random.seed(seed)
        self.se = seed
        ...
        self.action_space = {i: spaces.Box(low=-1, high=1, shape=(2,), dtype=np.float64) for i in self.agents}
        # The observation will be the coordinate of the agent
        # this can be described both by Discrete and Box space
        self.observation_space = {i: spaces.Box(low=-1000, high=1000, shape=(3,), dtype=np.float64) for i in self.agents}
    ...
    def compute_reward(self, action):
        ...
        :return:
        """
        power_selection = np.array(action.copy())
        sb_selection = power_selection * (self.V2I_channels_with_fast_fading ** 2)
        snrAll = sb_selection / self.sig2
        row, col = linear_sum_assignment(snrAll, True)
        snrPerVeh = snrAll[row, col]
        self.out_g = self.V2I_channels_with_fast_fading[row, col].reshape(self.n_Veh, 1)
        
        v2i_rate_list = np.zeros((self.n_Veh, 1))

        for n in range(self.n_Veh):
            v2i_rate_list[n][0] = np.log2(1 + snrPerVeh[n])
    ...
    def step(self, action):
        ...
        reward_sums = self.compute_reward(power)  # 计算出采取了action之后各个信道的容量
        ...
        return self.s_, reward_sums, done, {}
    ...
    def reset(self):
        ...
        self.renew_channels_fast_fading()
        return np.hstack((self.accumulation_rate, self.mutual_information, self.V2I_channels_with_fast_fading[:, :1]))